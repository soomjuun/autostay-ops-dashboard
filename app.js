...
function loadSheet(gid) {
  return new Promise((resolve, reject) => {
    ...
    const tm = setTimeout(()=>{ cleanup(); reject(new Error(`timeout:${gid}`)); }, 18000);
    function cleanup(){ clearTimeout(tm); delete window[cb]; sc.remove(); }
    window[cb] = payload => {
      cleanup();
      if (!payload || payload.status !== 'ok') { reject(new Error(`err:${gid}`)); return; }
      resolve((payload.table.rows||[]).map(r=>(r.c||[]).map(c=>c?c.v??'':'')));
    };
    sc.onerror = ()=>{ cleanup(); reject(new Error(`load:${gid}`)); };
...
function parseOverall(salesR, subR, mrrR) {
  const sQ1 = mRows(salesR, qIdx(salesR,'Q1')), sQ2 = mRows(salesR, qIdx(salesR,'Q2'));
  const bQ1 = mRows(subR,   qIdx(subR,  'Q1')), bQ2 = mRows(subR,   qIdx(subR,  'Q2'));
  const mQ1 = mRows(mrrR,   qIdx(mrrR,  'Q1')), mQ2 = mRows(mrrR,   qIdx(mrrR,  'Q2'));
...
function parseOps(rows) {
  ...
  for (let i = idx+1; i < rows.length; i++) {
    ...
    const retained   = num(rows[i][10]);
    const cancelSubs = num(rows[i][12]);
    const usageVal   = num(rows[i][7]);
    const utilizationVal = pct(rows[i][8]);

    // ★ 환불율 안전 파싱 ──────────────────────────────────────────
    // ops 집계 시트 col[9]에 환불 건수(raw count)가 저장된 경우,
    // pct()가 >3 값을 그대로 반환 → 141%, 333% 등 비정상 표기 발생.
    // 대응: 100% 초과 시 count로 간주 → usage 대비 비율 재산출.
    //       usage도 없으면 0 처리 후 _refundParseFlag 세팅.
    const _refundRaw = pct(rows[i][9]);
    let refundRate = 0;
    let _refundParseFlag = false;
    if (_refundRaw > 100) {
      const refundCount = num(rows[i][9]);
      const calcRate    = usageVal > 0 ? (refundCount / usageVal * 100) : 0;
      if (calcRate > 0 && calcRate <= 100) {
        refundRate = calcRate;           // 재산출 성공 → 사용
      } else {
        refundRate = 0;                  // 재산출 불가 → 0으로 표시
        _refundParseFlag = true;         // 원천 형식 경고 플래그
      }
    } else {
      refundRate = _refundRaw;           // 정상 범위 → 그대로 사용
    }

    // ★ 이탈률 안전 파싱 ─────────────────────────────────────────
    // ops 집계 시트 col[16]에 구독자 수(raw count)가 저장된 경우,
    // pct()가 >3 값을 그대로 반환 → 20403%, 49934% 등 비정상 표기 발생.
    // 대응: 100% 초과 시 count로 간주 → cancelSubs/retained 폴백.
    let churn = 0;
    let _churnParseFlag = false;
    if (rows[i][16] !== undefined) {
      const churnColPct = pct(rows[i][16]);
      if (churnColPct > 0 && churnColPct <= 100) {
        churn = churnColPct;             // 정상 범위 → 시트 값 사용
      } else if (churnColPct > 100) {
        _churnParseFlag = true;          // 비정상 → 폴백 + 경고 플래그
      }
    }
    // 폴백: 해지/유지 비율로 이탈률 계산
    if (churn === 0 && retained > 0 && cancelSubs > 0) {
      churn = cancelSubs / retained * 100;
    }

    // 설계 Capacity (보정값 기준, raw × 0.85)
    // 없으면 usage/utilization 역산 후 0.85 보정
    ...
    items.push({
      name,
      target:        num(rows[i][1]), gross:         num(rows[i][2]),
      achievement:   pct(rows[i][3]), net:           num(rows[i][4]),
      discountAmount:num(rows[i][5]), discountShare: pct(rows[i][6]),
      usage:         usageVal,        utilization:   utilizationVal,
      refundRate,    retained,
      newSubs:       num(rows[i][11]),cancelSubs,
      netAdds:       num(rows[i][13]),arpu:          num(rows[i][14]),
      status:        tx(rows[i][15]),
      churn,
      // ★ 파싱 이상 플래그 — runAudit()에서 분류 표시용
      _refundParseFlag,
      _churnParseFlag,
      _refundRawVal: _refundParseFlag ? num(rows[i][9]) : 0,
      // ★ 가동률 심층 분석
      rawCapacity:   rawCap,
      capacity:      cap,
      idleCount,
...
function runAudit(months, opsStores) {
  ...
  months.forEach(m => {
    ...
    // 가동률 이상치: 보정 Capacity(×0.85) 기준 130% 초과는 raw 데이터 점검 수준
  ...
  opsStores.forEach(s => {
    ...
    if (s._refundParseFlag) {
      // 컬럼 형식 오류 → 운영 리스크 아님, 시트 형식 확인 필요
    ...
    if (s._churnParseFlag) {
      // 컬럼 형식 오류 → 계산값으로 대체 표시, 시트 형식 확인 필요
    ...
    // ── ARPU ─────────────────────────────────────────────────────
    // MRR과 유지구독자가 모두 실데이터가 있는데 ARPU가 0인 경우만 flag
...
function renderGauges(ent) {
  ...
  makeGauge('gsvg-ach',  'gval-ach',  'gsub-ach',
    ach, achLabel, `목표 ${fmtS(c.target||0)} 대비`);
  makeGauge('gsvg-util', 'gval-util', 'gsub-util',
    util, utilLabel, `총사용 ${fmtN(c.usage||0)}회`);
  makeGauge('gsvg-churn','gval-churn','gsub-churn',
    churnH, fmtP(c.churn||0), `이탈률 (낮을수록 건강)`);
  makeGauge('gsvg-mrr',  'gval-mrr',  'gsub-mrr',
    mrrM, `${(c.mrrYoY||0)>0?'+':''}${fmtP(c.mrrYoY||0)}`, `MRR ${fmtS(c.mrr||0)}`);
}
...
function renderInsights(ent) {
  ...
  const topStore = ent.isAll
    ? [...dashboard.opsStores].sort((a,b)=>b.gross-a.gross)[0] : null;
...
function renderBenchmarkChart(ent) {
  // HTML 랭킹 스트립으로 렌더링 (rankStrip 엘리먼트 사용)
  const el = $('rankStrip');
  if (!el) return;
  const sorted = [...dashboard.opsStores].sort((a,b)=>b.gross-a.gross);
  ...
  el.innerHTML = sorted.map((s,i)=>{
    ...
    const achColor = ach>=100?'#216552':ach>=80?'#c07b48':'#b24c58';
    ...
    return `<div class="rank-row${isSelected?' selected':''}" data-store="${s.name}" role="button" tabindex="0"
      ...
      <span class="rank-ach" style="color:${achColor}">${fmtP(ach)}</span>
    </div>`;
  }).join('');
...
function renderHealthChart(ent) {
  ...
  dashboard.stores.forEach(s=>{
    const sc = aggMonths(filterMonths(s.months))||{};
    avgVals[0] += Math.min(100,sc.achievement||0)/dashboard.stores.length;
    avgVals[1] += Math.min(100,sc.utilization||0)/dashboard.stores.length;
    avgVals[2] += Math.max(0,100-(sc.churn||0)*5)/dashboard.stores.length;
    avgVals[3] += Math.max(0,100-(sc.refundRate||0)*3)/dashboard.stores.length;
    avgVals[4] += Math.max(0,Math.min(100,50+(sc.mrrYoY||0)))/dashboard.stores.length;
    avgVals[5] += Math.max(0,Math.min(100,50+(sc.grossYoY||0)))/dashboard.stores.length;
  });
...
function renderHeroKpis(ent) {
  ...
  // 현재 필터 상태
  const storeStr  = ent.isAll ? '전체 6개 매장 합산' : ent.name;
...
function buildCapacityData(ent) {
  ...
  if (ent.isAll) {
    // store key(name) 기준 — 순서·위치 의존 없음
    return dashboard.stores.map(s =>
...
function renderCapacityPanel(ent) {
  ...
  <div class="cap-summary">
    <div class="cap-sum-item">
      <div class="cap-sum-label">확정 가동률${capTip('확정월 총사용 ÷ 확정월 보정 Capacity × 100<br>마감 완료 월만 적용 (풀 월 기준)')}</div>
      <div class="cap-sum-val" style="color:${utilColor(confUtil)}">${totConfCap>0?fmtP(confUtil):'—'}</div>
  ...
  [...stores].sort((a,b) => {
    ...
    html += `<div class="cap-store-card">
      <div class="cap-store-head">
        ...
        <span class="cap-store-util" style="color:${bColor}">${fmtP(dispUtil)} ${sBadge(dispStatus)}</span>
      </div>
...
function renderTable(ent) {
  ...
  const rows = tableStores.map(s=>{
    const ach = s.achievement||0;
    const cls = ach>=100?'good':ach>=80?'warn':'bad';
    const selected = s.name===selName?'selected':'';
    return `<tr class="${selected}" data-store="${s.name}">
      <td>${s.name}</td>
      <td>${fmtS(s.gross)}</td>
      <td><div class="ach-wrap">${fmtP(ach)}<div class="ach-bar-bg"><div class="ach-bar-fill" style="width:${Math.min(100,ach)}%;background:${ach>=100?'#216552':ach>=80?'#c07b48':'#b24c58'}"></div></div></div></td>
      <td>${fmtS(s.net)}</td>
      <td>${fmtN(s.usage)}</td>
      <td>${fmtP(s.utilization||0)}</td>
      <td>${fmtP(s.refundRate||0)}</td>
      <td>${(s.netAdds||0)>=0?'+':''}${fmtN(s.netAdds||0)}</td>
      <td>${fmtS(s.arpu)}</td>
      <td><span class="verdict-chip ${cls}">${s.status}</span></td>
    </tr>`;
  }).join('');
...
async function init(showLoading=true) {
  ...
  const errBannerEl = $('errBanner'); // errBanner는 CSS에서 display:none으로 시작
  ...
  try {
    ...
    $('loadingOverlay').style.display = 'none';
    const errEl = $('errBanner') || $('errorMsg');
    if (errEl) { errEl.style.display = 'block'; errEl.textContent = `⚠ 데이터 로드 실패: ${e.message}. 잠시 후 새로고침 버튼을 눌러 재시도하세요.`; }
  }
}

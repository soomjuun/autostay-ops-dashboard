...
function renderBenchmarkChart(ent) {
  ...
  el.innerHTML = sorted.map((s,i)=>{
    ...
    // ★ Change 4: mini progress bar under achievement
    const achBarW  = Math.min(100, ach).toFixed(1);
    const achBarColor = ach>=100?'#216552':ach>=80?'#c07b48':'#b24c58';
  ...
  el.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
      ...
      <div style="background:var(--surface-soft);border:1px solid var(--border);border-left:3px solid var(--amber);border-radius:var(--r-md);padding:14px 16px">
        <div style="font-size:10.5px;font-weight:700;color:var(--amber);text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px">자발적 이탈</div>
        <div style="font-size:22px;font-weight:900;color:var(--text);margin-bottom:2px">${fmtN(voluntary)}명</div>
      ...
      <div style="background:var(--surface-soft);border:1px solid var(--border);border-left:3px solid var(--rose);border-radius:var(--r-md);padding:14px 16px">
        <div style="font-size:10.5px;font-weight:700;color:var(--rose);text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px">비자발적 이탈 (결제실패)</div>
        <div style="font-size:22px;font-weight:900;color:var(--text);margin-bottom:2px">${fmtN(involuntary)}명</div>
    ...
    <div style="margin-bottom:10px">
      <div style="display:flex;height:18px;border-radius:99px;overflow:hidden;gap:2px">
        <div style="width:70%;background:var(--amber);display:flex;align-items:center;justify-content:center">
          <span style="font-size:10px;font-weight:800;color:#fff">자발 70%</span>
        </div>
        <div style="width:30%;background:var(--rose);display:flex;align-items:center;justify-content:center">
          <span style="font-size:10px;font-weight:800;color:#fff">결제 30%</span>
        </div>
      </div>
    </div>

    <!-- 면책 고지 -->
    <div style="padding:8px 12px;background:var(--amber-soft);border:1px solid rgba(192,123,72,.2);border-radius:var(--r-sm);font-size:11px;color:var(--muted);line-height:1.6">
      ※ 실데이터 미분리 · 업계 평균 기준 추정 (자발 70% / 결제실패 30%)
      — 자동차 세차 구독 서비스 업계 이탈 유형 평균 기반. 실 데이터 분리 시 이 섹션을 업데이트하세요.
    </div>
  `;
}
